"""
Drift Monitor — Live Simulation Dashboard
==========================================
Single-dataset Streamlit app for the thesis defense demo.
Monitors data drift using KS, PSI and Wasserstein tests.
...
import sys, os
# Add parent directory to path so we can import predictor modules
...
from predictor.engine.cleaner  import DataCleaner   # data preprocessing
from predictor.engine.trainer  import ModelTrainer   # model training + champion selection
from predictor.engine.detector import DriftDetector  # KS + PSI + Wasserstein
from predictor.engine.store    import ModelStore      # save models to disk
from predictor.config          import ALGORITHMS      # algorithm registry

# ── Directory setup ─────────────────────────────────────────────────
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(os.path.dirname(BASE_DIR), "drift_mlops", "real_data")
SAVED_DIR= os.path.join(BASE_DIR, "saved_sim")
os.makedirs(SAVED_DIR, exist_ok=True)  # create saved_sim/ if it doesn't exist

TARGET = "target"  # name of the prediction target column in all CSVs

# ── Dataset registry ─────────────────────────────────────────────────
# Each entry defines file paths, display labels, and metadata
# sim2_app.py uses USE_CASES from config.py — this is the single-dataset version
DATASETS = {
    "Bike Sharing (2011)": {
        "ref":        os.path.join(DATA_DIR, "bike_reference.csv"),   # Spring — training
        "stream1":    os.path.join(DATA_DIR, "bike_new_early.csv"),   # Summer — ground truth
        "stream2":    os.path.join(DATA_DIR, "bike_new_late.csv"),    # Autumn — future
        "unit":       "bikes/h",
        "phase1":     "Summer",
        "phase2":     "Autumn",
        "ref_label":  "Spring 2011",
        "stream_start": "2011-06-01",
        "baseline":   63.8,   # persistence baseline MAE for comparison
    ...
    "Custom Upload": {
        # User can upload their own CSVs
        "ref":        None,
        "stream1":    None,
        "stream2":    None,
        "unit":       "",
...
# ── Module instances ─────────────────────────────────────────────────
# Created once at module level — reused across all Streamlit reruns
trainer  = ModelTrainer()
detector = DriftDetector()
store    = ModelStore(SAVED_DIR)

# ── Simulation constants ─────────────────────────────────────────────
MIN_RETRAIN_SAMPLES = 500   # minimum labelled observations before retraining
COOLDOWN_STEPS      = 6     # steps to wait after a retrain before allowing another
MEDIUM_ALERT_STEPS  = 8     # consecutive MEDIUM severity steps to trigger retrain

# ═══════════════════════════════════════════════════════════════════
# PAGE CONFIG & CSS
# Custom dark theme — IBM Plex Mono for monospace elements
# ═══════════════════════════════════════════════════════════════════
...
# All CSS is injected as raw HTML — Streamlit has no native theming API
st.markdown("""
<style>
...
.stApp { background: #111014; }
#MainMenu, footer, header { visibility: hidden; }
[data-testid="collapsedControl"] { display: none; }
.block-container { padding: 0 !important; max-width: 100% !important; }
.topbar {
    background: #7c1c2e; height: 44px;
    display: flex; align-items: center; justify-content: space-between;
    padding: 0 24px; border-bottom: 1px solid #5c1220;
}
.topbar-logo {
    font-family: 'IBM Plex Mono', monospace; font-size: 12px;
    font-weight: 500; color: #ffeef2; letter-spacing: 3px;
    text-transform: uppercase; padding-right: 20px; margin-right: 20px;
    border-right: 1px solid rgba(255,238,242,0.2);
}
.topbar-sub { font-size: 11px; color: rgba(255,238,242,0.55); letter-spacing: 0.2px; }
.topbar-right {
    font-family: 'IBM Plex Mono', monospace; font-size: 10px;
    color: rgba(255,238,242,0.4); letter-spacing: 0.5px;
}
.stat-strip {
    background: #1a1720; border-bottom: 1px solid #2a2730;
    display: grid; grid-template-columns: repeat(7, 1fr); padding: 0 24px;
}
.stat { padding: 12px 16px 12px 0; border-right: 1px solid #2a2730; }
.stat:last-child { border-right: none; }
.stat-label {
    font-size: 9px; text-transform: uppercase; letter-spacing: 1.2px;
    color: #5a5660; margin-bottom: 4px;
    font-family: 'IBM Plex Mono', monospace;
}
.stat-value {
    font-family: 'IBM Plex Mono', monospace; font-variant-numeric: tabular-nums;
    font-size: 22px; font-weight: 500; color: #e8e4de; line-height: 1;
}
.stat-sub {
    font-size: 10px; color: #5a5660; margin-top: 4px;
    font-family: 'IBM Plex Mono', monospace; font-variant-numeric: tabular-nums;
}
...
.log-wrap {
    background: #0d0c10; border: 1px solid #2a2730; border-radius: 2px;
    padding: 8px 12px; max-height: 480px; overflow-y: auto;
}
.log-row {
    display: grid; grid-template-columns: 60px 1fr; gap: 10px;
    padding: 4px 0; border-bottom: 1px solid #1a1720;
...
.info-panel {
    background: #1a1720; border-left: 2px solid #c2415a;
    padding: 10px 14px; font-size: 12px; color: #9a9598;
    line-height: 1.7; margin: 8px 0; border-radius: 0 2px 2px 0;
}
.rule { border: none; border-top: 1px solid #2a2730; margin: 16px 0; }
section[data-testid="stSidebar"] { background: #1a1720; border-right: 1px solid #2a2730; }
.stButton > button {
    background: #1a1720 !important; border: 1px solid #2a2730 !important;
    color: #e8e4de !important; font-family: 'IBM Plex Mono', monospace !important;
    font-size: 11px !important; border-radius: 2px !important; letter-spacing: 0.3px;
}
.stButton > button[kind="primary"] {
    background: #7c1c2e !important; border-color: #7c1c2e !important; color: #ffeef2 !important;
}
.stTextArea textarea {
    background: #0d0c10 !important; border-color: #2a2730 !important;
    color: #5a5660 !important; font-family: 'IBM Plex Mono', monospace !important;
...
# ── Plotly chart theme ───────────────────────────────────────────────
# Applied to every chart via **PLOT unpacking
PLOT = dict(
...
# Severity level → (color, pill class) mapping
# Used to color drift score display and log entries
SEV = {
    "none":     ("#4a9eda", "ok"),
    "low":      ("#4a9eda", "ok"),
    "medium":   ("#d4812a", "warn"),
...
CHART_CFG = {"displayModeBar": False}  # hide plotly toolbar

# ── UI helpers ───────────────────────────────────────────────────────

def sec(title):
    """Render a section label above a content block."""
    st.markdown(f'<div class="section-label">{title}</div>', unsafe_allow_html=True)

def show(fig, height=None):
    """Render a Plotly figure with standard config."""
    if height:
...
def stat_html(label, value, sub="", color="#e8e4de", border=True):
    """
    Generate HTML for a single stat box in the metrics strip.
    Returns raw HTML string — injected via st.markdown(unsafe_allow_html=True).
    """
...
# Session state is the only way to persist data across reruns.
# Without it: model, stream position, history — everything resets.
# ═══════════════════════════════════════════════════════════════════

DEFAULTS = {
    "sim_model":           None,   # TrainResult — current champion model
    "sim_all_results":     None,   # Dict[str, TrainResult] — all algorithms
    "sim_champion":        None,   # str — champion algorithm name
    "sim_ref_df":          None,   # DataFrame — reference distribution for drift
    "sim_stream":          None,   # DataFrame — full stream (summer + autumn)
    "sim_stream_idx":      0,      # int — current position in stream
    "sim_phase":           "stream1",  # "stream1" (summer) or "stream2" (autumn)
    "sim_history":         [],     # List[Dict] — one entry per step
    "sim_events":          [],     # List[Dict] — log entries
    "sim_running":         False,
    "sim_retrains":        0,      # total number of retrains triggered
    "sim_ref_mean":        0.0,    # training mean — for mean baseline
    "sim_feature_cols":    [],     # feature column names from training
    "step_size":           24,     # hours per simulation step
    "drift_threshold":     0.5,    # HIGH threshold (adjustable in sidebar)
    "sim_champion_history":[],     # retrain events with champion info
    "sim_samples_seen":    0,      # cumulative labelled observations
    "ds_unit":             "bikes/h",
    "ds_phase1":           "Summer",
    "ds_phase2":           "Autumn",
    "ds_ref_label":        "Spring 2011",
    "ds_stream_start":     "2011-06-01",
    "ds_baseline":         63.8,
    "ds_name":             "Bike Sharing (2011)",
}

# Only initialise keys that don't already exist
# This preserves state across reruns
...
def log(msg, kind="ok"):
    ...
    Entries are shown in the right panel in reverse chronological order.
    kind: "ok" | "warn" | "alert" | "retrain"
    """
    ts = datetime.now().strftime("%H:%M:%S")
    st.session_state.sim_events.insert(0, {"ts": ts, "msg": msg, "kind": kind})
    # Keep only last 300 entries to avoid memory growth
...
def do_train(df, label=""):
    """
    Train all three algorithms on the reference dataset.
    Returns the results dict, champion name, feature columns, and training mean.

    Called once at simulation start on the Spring data.
    Also called indirectly by do_retrain() via trainer.train_all().
    """
    cleaned = DataCleaner.clean(df, TARGET)
    # Feature columns = all columns except target and internal _task marker
    fc = [c for c in cleaned.columns if c not in [TARGET, "_task"]]
    X, y = cleaned[fc], cleaned[TARGET]
    # train_all() trains RF, XGBoost and Ridge in sequence
    results = trainer.train_all(X, y, task="regression")
    # Champion = highest held-out R²
    champ   = trainer.get_champion(results, metric="r2")
    store.save(results, champ, label)  # persist to disk
    return results, champ, fc, float(y.mean())


def predict_batch(batch_df):
    """
    Generate predictions for one batch (24 hours by default).
    Uses the current champion model from session state.
    TARGET column is never included — no lookahead.
    """
    r  = st.session_state.sim_model       # TrainResult
    fc = st.session_state.sim_feature_cols
    # Select only feature columns — drop target if present
    X  = batch_df[[c for c in fc if c in batch_df.columns]]
    # Apply scaler for Ridge, pass raw values for tree models
...
def do_retrain():
    """
    Expanding window retrain: Spring data + all labelled stream observations.
    Updates session state with new champion model and new reference distribution.

    After retraining, the reference distribution is reset to accumulated stream data.
    This causes the drift score to collapse on the next step — expected behaviour.

    Returns:
        (success: bool, n_samples: int)
    """
    S       = st.session_state
    seen    = S.sim_stream.iloc[:S.sim_stream_idx]
    # Only rows with ground truth labels
    seen_gt = seen.dropna(subset=[TARGET]) if TARGET in seen.columns else pd.DataFrame()

    # Guard: don't retrain on too few samples
    if len(seen_gt) < MIN_RETRAIN_SAMPLES:
        return False, len(seen_gt)

    # Combine Spring reference + labelled stream observations
    ...
    # Reset reference distribution to accumulated stream data
    # Next step's drift will be measured against THIS, not Spring
    # This is why drift collapses to ~0.03 after retrain
    S.sim_ref_df = seen_gt[[c for c in fc if c in seen_gt.columns]].copy()

    # Record this retrain event for the champion timeline table
...
def process_step():
    """
    Core simulation loop — runs once per "Step →" click or auto-run tick.

    Sequence:
        1. Advance stream by one batch (default 24h)
        2. Generate predictions (no lookahead)
        3. Read actual values if ground truth available (Summer phase)
        4. Compute drift score over cumulative window
        5. Evaluate retrain conditions
        6. Log the step
        7. Append to history for chart rendering

    Returns False when stream is exhausted.
    """
    S    = st.session_state
    stream, idx, step = S.sim_stream, S.sim_stream_idx, S.step_size

    # Stop if stream is exhausted
    if stream is None or idx >= len(stream):
        return False

    # Step 1: Advance stream
    batch = stream.iloc[idx:idx+step].copy()
    S.sim_stream_idx += step

    # Calculate display date from stream start + elapsed hours
    ...
    # Determine phase: stream1=Summer (has labels), stream2=Autumn (no labels)
    ...
    # Step 2: Predict — target column is excluded inside predict_batch()
    try:
        y_pred    = predict_batch(batch)
        pred_mean = round(float(y_pred.mean()), 1)  # daily average for display
    except Exception as e:
        log(f"prediction error — {e}", "alert")
        return True

    # Step 3: Read actual values (Summer only)
    ...
    if has_target:
        ...
        if len(y_true) > 0:
            ...
            # MAE = mean of hourly errors (not daily average errors)
            mae         = round(float(mean_absolute_error(y_true, y_p)), 1)
        S.sim_samples_seen = S.get("sim_samples_seen", 0) + len(y_true)

    # Step 4: Drift detection over cumulative window
    # Max lookback = 200 × step_size hours (well beyond summer length)
    window = stream.iloc[max(0, idx - 200*max(step,1)):idx+step]
    # Only use features present in both reference and window
    common = [c for c in S.sim_feature_cols
              if c in S.sim_ref_df.columns and c in window.columns]

    drift_score, severity = 0.0, "none"
    if common and len(window) >= 50:  # minimum 50 rows for reliable statistics
        try:
    ...
    # Find the step number of the last retrain (0 if none)
    ...
    # Count consecutive MEDIUM steps in recent history
    ...
    # Cooldown active — log alert but don't retrain
    ...
    # Autumn phase — drift detected but no labels available
    ...
    # Execute retrain if conditions met
    if should_retrain:
        if samples_seen < MIN_RETRAIN_SAMPLES:
            # Not enough data yet — log accumulation progress
            event   = "ALERT"
            act_str = f" · actual={actual_mean}" if actual_mean else ""
            mae_str = f" · mae={mae}" if mae is not None else ""
            log(f"step {step_num:03d} · {date_str} · {retrain_reason} · accumulating ({samples_seen}/{MIN_RETRAIN_SAMPLES}) · pred={pred_mean}{act_str}{mae_str}", "warn")
        else:
            event = "RETRAIN"
    ...
    # Step 6: Log normal step (no event triggered)
    if event is None:
        act     = f" · actual={actual_mean}" if actual_mean else (
                  f" · {PHASE2_LABEL} (future)" if S.sim_phase == "stream2" else "")
        sev_tag = f" [{severity}]" if severity not in ["none", "low"] else ""
        mae_str = f" · mae={mae}" if mae is not None else ""
        log(f"step {step_num:03d} · {date_str} · drift={drift_score:.3f}{sev_tag} · pred={pred_mean}{act}{mae_str}", "ok")

    # Step 7: Append to history — charts read from this list
...
with st.sidebar:
    ...
    if model_ready and champ:
        ...
        st.markdown(
            f'<span class="pill pill-ok">active</span>&nbsp;'
            f'<span class="num" style="font-size:11px;">{label}</span><br>'
            f'<span style="color:#5a5660;font-size:10px;">R²&nbsp;{r2}&nbsp;·&nbsp;held-out</span>',
            unsafe_allow_html=True)
    else:
        st.markdown('<span class="pill pill-mute">no model</span>', unsafe_allow_html=True)

    # Batch size selector — how many hours per step
    ...
    # Drift threshold slider — adjustable HIGH threshold
...
# ═══════════════════════════════════════════════════════════════════
# TOP BAR
# Shows dataset name, current phase, progress, retrain count
# ═══════════════════════════════════════════════════════════════════
...
# Thin red progress bar below topbar
if model_ready and st.session_state.sim_history:
    _pct_w = int(min(st.session_state.sim_stream_idx /
                     max(len(st.session_state.sim_stream), 1), 1) * 100)
    st.markdown(f"""
...
# ═══════════════════════════════════════════════════════════════════
# CONTROL BAR
# Dataset selector + Start button (before model trained)
# Step / Auto / Batch / Download / Reset (after model trained)
# ═══════════════════════════════════════════════════════════════════

st.markdown('<div style="background:#1a1720;border-bottom:1px solid #2a2730;padding:8px 24px;">', unsafe_allow_html=True)

if not model_ready:
    # Pre-simulation: dataset selection and start button
    _c1, _c2, _c3 = st.columns([2, 2, 4])
    with _c1:
        ds_name = st.selectbox("Dataset", list(DATASETS.keys()),
                               label_visibility="collapsed", key="ds_selector")
        ds = DATASETS[ds_name]

    with _c2:
        if ds_name == "Custom Upload":
            # Custom upload mode — user provides their own CSVs
            ...
            # Predefined dataset — check files exist on disk
    ...
    with _c3:
        ...
        if st.button("▶  Start Simulation", type="primary",
                     use_container_width=True, disabled=not has_demo):
            with st.spinner(f"Training on {ds['ref_label'] if ds_name != 'Custom Upload' else 'reference data'}..."):
                ...
                if ds_name == "Custom Upload":
                    ...
                    stream2 = pd.read_csv(stream2_file).drop(columns=[TARGET], errors="ignore") \
                              if stream2_file else pd.DataFrame()
                    baseline = None
                else:
                    ref      = pd.read_csv(ds["ref"])
                    stream1  = pd.read_csv(ds["stream1"])
                    # Remove target from autumn — no ground truth in future phase
                    stream2  = pd.read_csv(ds["stream2"]).drop(columns=[TARGET], errors="ignore") \
                               if ds["stream2"] else pd.DataFrame()
                    baseline = ds["baseline"]

                # Train initial model on reference data
                results, champ, fc, ref_mean = do_train(
                    ref, ds["ref_label"] if ds_name != "Custom Upload" else "Reference"
                )

                # Concatenate summer + autumn into one stream
                stream = pd.concat([stream1, stream2], ignore_index=True) \
                         if not stream2.empty else stream1
                ref_c  = DataCleaner.clean(ref, TARGET)

                # Write everything to session state
                st.session_state.update({
                    "sim_all_results":  results,
                    "sim_champion":     champ,
                ...
                log(f"Stream: {len(stream1):,} rows {p1_inp} (ground truth)" +
                    (f" + {len(stream2):,} rows {p2_inp} (future)" if not stream2.empty else ""), "ok")
            st.rerun()

else:
    # Post-training control bar
    ...
    with _b2:
        # Auto-run toggle — calls process_step() every 150ms
        auto = st.toggle("Auto", key="_auto")
    with _b3:
        # Batch size — how many hours per step
        ...
        st.session_state.step_size = step_size
    with _b4:
        # Download full event log as text file
        if st.session_state.sim_events:
            log_text = "\n".join([f"{e['ts']} {e['msg']}"
                                  for e in reversed(st.session_state.sim_events)])
    ...
    with _b6:
        ...
        st.markdown(
            ...
            f'<span style="font-size:9px;color:#3a3740;text-transform:uppercase;'
            f'letter-spacing:1px;margin-left:5px;">retrains</span>'
            f'</div>', unsafe_allow_html=True)

    # Auto-run: process one step then rerun after short delay
    if auto and not is_done:
        import time; process_step(); time.sleep(0.15); st.rerun()

st.markdown('</div>', unsafe_allow_html=True)

# Show instructions before simulation starts
if not model_ready:
    st.markdown("""
    <div class="info-panel">
    <strong style="color:#e8e4de;">How this works</strong> —
    A model is trained on Spring 2011 bike rental data (March–May, 2,193 hours).
    Summer 2011 data then streams in, validated against ground truth.
    Drift is detected using KS, PSI and Wasserstein tests.
    When drift exceeds the threshold and sufficient data has accumulated,
...
if not history:
    ...
    <div style="padding:60px 0;text-align:center;color:#3a3740;">
        ...
        <div style="font-size:13px;color:#5a5660;">
            Press <strong style="color:#e8e4de;">Step →</strong> or enable
            <strong style="color:#e8e4de;">Auto</strong>
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
# ═══════════════════════════════════════════════════════════════════
# METRICS STRIP
# Seven stat boxes: prediction, actual, drift, R², MAE, retrains, champion
# ═══════════════════════════════════════════════════════════════════

df_h      = pd.DataFrame(history)
latest    = history[-1]
summer_df = df_h[df_h["phase"]=="stream1"]   # labelled steps only
autumn_df = df_h[df_h["phase"]=="stream2"]   # future steps only
rt_df     = df_h[df_h["event"]=="RETRAIN"]   # retrain events only
...
# Average MAE across all labelled steps (used in summary table and retrain impact chart)
avg_mae = round(float(summer_df["mae"].dropna().mean()), 1) \
          if not summer_df.empty and summer_df["mae"].notna().any() else None

# Current step MAE — shown in stat strip
current_mae = latest.get("mae", None)
...
# Daily toggle — must come BEFORE actual_html is built
show_daily = st.toggle("Show daily totals (×24)", key="daily_toggle")

# Determine display values based on toggle
if show_daily:
    pred_display   = f"{round(pred * 24):,}"
    actual_display = f"{round(actual * 24):,}" if actual else None
    unit_display   = "bikes/day"
else:
    pred_display   = f"{pred:.0f}"
    actual_display = f"{actual:.0f}" if actual else None
    unit_display   = UNIT

# Actual stat box — shows ground truth in summer, "AUTUMN" pill in autumn
if actual:
    dc = "stat-up" if actual >= pred else "stat-down"
    if show_daily:
        diff_display = f"{round((actual - pred) * 24):+,}"
    else:
        diff_display = f"{actual-pred:+.0f}"
    actual_html = (
        f'<div class="stat-value">{actual_display}</div>'
        f'<div class="stat-sub {dc}">{diff_display} · {err_pct}%</div>'
    )
else:
    actual_html = (f'<div class="stat-value" style="color:#5a5660;font-size:12px;padding-top:6px;">'
                   f'<span class="pill pill-future">{PHASE2}</span></div>'
                   f'<div class="stat-sub">no ground truth</div>')

# MAE stat box — shows current step MAE vs baseline
if current_mae is not None:
    vs = round(BASELINE - current_mae, 1) if BASELINE else None
    mae_html = (f'<div class="stat-value">{current_mae:.1f}</div>'
                f'<div class="stat-sub {"stat-up" if vs and vs>0 else "stat-down" if vs else ""}">'
                f'{"vs baseline "+f"{vs:+.1f} {UNIT}" if vs else UNIT}</div>')
else:
    mae_html = f'<div class="stat-value" style="color:#5a5660;">—</div><div class="stat-sub">{PHASE1} only</div>'

# Render the 7-column metrics strip
st.markdown(
    ...
    + stat_html("Prediction", pred_display, unit_display)
    + f'<div class="stat"><div class="stat-label">Actual</div>{actual_html}</div>'
    + stat_html("Drift Score", f"{drift:.3f}",
                f'<span class="pill pill-{sev_pill}">{sev}</span>', sev_color)
    + stat_html("Model R²",    str(champ_r2), "held-out test")
    + f'<div class="stat"><div class="stat-label">MAE</div>{mae_html}</div>'
    + stat_html("Retrains",    str(n_ret), "auto-triggered",
                "#c2415a" if n_ret > 0 else "#5a5660")
...
# ═══════════════════════════════════════════════════════════════════
# CHARTS + EVENT LOG
# Left 3/4: charts | Right 1/4: scrollable event log
# ═══════════════════════════════════════════════════════════════════

col_left, col_right = st.columns([3,1], gap="small")

with col_left:
    # ── Chart 1: Prediction vs Actual ───────────────────────────────
    sec("Prediction vs Actual")
    fig = go.Figure()

    # Reference mean horizontal line
    ...
    # Summer: solid prediction line + actual dots
    ...
    # Autumn: dashed prediction line (no actual dots)
    ...
    # Vertical red lines at retrain events
    ...
    # Vertical grey line at summer/autumn boundary
    ...
    # ── Charts 2 + 3: Drift Score | MAE per Step ────────────────────
    ...
    with dc1:
        ...
        # Threshold lines (LOW=0.15 blue, MEDIUM=0.30 orange, HIGH=threshold red)
        ...
        # Triangle markers at retrain events
        if not rt_df.empty:
    ...
    with dc2:
        ...
        if not summer_df.empty and summer_df["mae"].notna().any():
            ...
            fig3  = go.Figure()
            # Color bars by MAE magnitude: blue=good, orange=medium, red=bad
            fig3.add_trace(go.Bar(
                x=summer_df.loc[mae_v.index,"step"], y=mae_v,
                marker_color=["#4a9eda" if v<20 else "#d4812a" if v<40 else "#c2415a"
                              for v in mae_v],
                marker_opacity=0.8))
            if BASELINE:
                fig3.add_hline(y=BASELINE, line_color="#3a3740", line_width=1,
                               line_dash="dot",
                               annotation_text=f"baseline {BASELINE}",
                               annotation_font=dict(size=8, color="#5a5660"))
            for _, r in rt_df.iterrows():
                fig3.add_vline(x=r["step"], line_color="#c2415a",
                               line_width=1, line_dash="dot")
            fig3.update_layout(**PLOT, showlegend=False,
                xaxis_title="step", yaxis_title=UNIT)
            show(fig3, 180)
        else:
            st.markdown(
                '<div style="height:180px;display:flex;align-items:center;'
                'justify-content:center;color:#5a5660;font-size:11px;">'
                'Waiting for ground truth</div>', unsafe_allow_html=True)

    # ── Charts 4 + 5: Retrain Impact | Champion Timeline ────────────
    if not rt_df.empty and avg_mae is not None:
        ...
        with rc1:
            ...
            for i, (_, row) in enumerate(rt_df.iterrows()):
                sr = row["step"]
                # 5-step window before and after each retrain
        ...
        with rc2:
            ...
            # Table showing which algorithm won at each retrain
...
with col_right:
    ...
    # Hidden text area — just for copy convenience
    st.text_area("Event Log", value=log_text, height=48,
                 label_visibility="collapsed", key="log_ta")

    # Render log entries as styled HTML rows
    # Colors: blue=retrain, red=alert, orange=warn, grey=normal
    for e in events[:80]:
...
# ═══════════════════════════════════════════════════════════════════
# RUN SUMMARY TABLE
# Shown at the bottom — totals, MAE vs baseline, algorithm comparison
# ═══════════════════════════════════════════════════════════════════
...
with s2:
    # MAE performance vs persistence baseline
    if avg_mae:
        rows2 = [{"Metric":"Avg MAE","Value":f"{avg_mae:.1f} {UNIT}"}]
        if BASELINE:
            rows2 += [
                {"Metric":"Baseline",      "Value":f"{BASELINE} {UNIT}"},
                {"Metric":"ML vs baseline","Value":f"{BASELINE-avg_mae:+.1f} {UNIT}"},
            ]
        st.dataframe(pd.DataFrame(rows2), use_container_width=True, hide_index=True)

with s3:
    # Algorithm comparison table — champion marked with ✓
    if st.session_state.sim_all_results and champ:
        st.dataframe(pd.DataFrame([
            {
